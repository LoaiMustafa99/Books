...
@endSection
@section("css-links")
    <link rel="stylesheet" href="{{asset("assets/css/en/pages/sub_categories.css")}}">
@endsection
@section("page-nav-title")
    <div class="app-title">
        <div>
            <h1><i class="fa fa-dashboard"></i>{{__("Sub Categories")}}</h1>
            <p>{{__("All Sub Categories")}}</p>
        </div>
                ...
                <div class="card-body">
                    ...
                    @foreach($subCategories as $sub)
                        <div class="card-text category-box">
                            <a href="{{(isset($subCategory) && $sub->level < $mainCategory->limit_levels_of_sub_categories) || (!isset($subCategory) &&  $mainCategory->limit_levels_of_sub_categories > 1)? route("admin.sub_category.index", ["main_id" => $mainCategory->id, "parent_id" => $sub->id]) : "#"}}" class="box-link">
                                {{$sub->name}}
                            </a>
